...
QString cache_main = QStandardPaths::writableLocation(QStandardPaths::CacheLocation) + "/baysal-kalem/";
...
DrawingWidget::DrawingWidget(QWidget *parent): QWidget(parent) {
    ...
    cropLayout->setContentsMargins(16, 16, 16, 16);
    cropLayout->setSpacing(0);
    cropWidget->hide();

    magnifierWidget = new MovableWidget(mainWidget);
    magnifierWidget->stackUnder(this);
    QBoxLayout* magLayout = new QVBoxLayout(magnifierWidget);
    magLayout->addWidget(magnifierWidget->crop);
    magLayout->setContentsMargins(16, 16, 16, 16);
    magLayout->setSpacing(0);
    magnifierWidget->hide();

    coordinateWidget = new MovableWidget(mainWidget);
    coordinateWidget->stackUnder(this);
    QBoxLayout* coordLayout = new QVBoxLayout(coordinateWidget);
    coordLayout->addWidget(coordinateWidget->crop);
    coordLayout->setContentsMargins(16, 16, 16, 16);
    coordLayout->setSpacing(0);
    coordinateWidget->hide();

    rulerWidget = new MovableWidget(mainWidget);
    rulerWidget->stackUnder(this);
    QBoxLayout* rulerLayout = new QVBoxLayout(rulerWidget);
    rulerLayout->addWidget(rulerWidget->crop);
    rulerLayout->setContentsMargins(16, 16, 16, 16);
    rulerLayout->setSpacing(0);
    rulerWidget->hide();

    protractorWidget = new MovableWidget(mainWidget);
    protractorWidget->stackUnder(this);
    QBoxLayout* protractorLayout = new QVBoxLayout(protractorWidget);
    protractorLayout->addWidget(protractorWidget->crop);
    protractorLayout->setContentsMargins(16, 16, 16, 16);
    protractorLayout->setSpacing(0);
