...
void updateGui(){
    ...
    if(toolButtons[ACIOLCER] != nullptr){
        bool aciolcerActive = drawing->protractorWidget && !drawing->protractorWidget->isHidden();
        QString style = aciolcerActive ? "background-color:"+drawing->penColor.name()+";" : "background-color: none;";
        toolButtons[ACIOLCER]->setStyleSheet(style);
    }
